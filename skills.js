...
function getLanguages(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    for (let x = 0; x < arr[i].languages.length; x++) {
      if (!languages.includes(arr[i].languages[x])) {
        languages.push(arr[i].languages[x]);
      }
    }
  }
  return languages;
...
// console.log(getLanguages(employees));

function getActiveEmployee(obj) {
  for (let i = 0; i < obj.length; i++) {
    if (obj[i].isActive === false) {
      console.log(obj[i].firstName);
    }
  }
}
// getActiveEmployee(employees);

function calculeSalaire(obj) {
  let salaries = 0;
  let count = 0;

  for (let i = 0; i < obj.length; i++) {
    if (obj[i].department === "Développement") {
      salaries += obj[i].salary;
      count++;
    }
  }
...
console.log(calculeSalaire(employees));