...
{
    fullname = obj.map(ele => `${ele.firstName} ${ele.lastName}`)
    return fullname;
}
// console.log(extractFullName(employees));

function emailSignature(obj)
{
    signatre = obj.map(ele => `${ele.firstName}-${ele.lastName}'@gmail.com'`)
    return signatre
}
// console.log(emailSignature(employees))

function increaseSalaries(obj)
{
    return obj.map(ele => ele.salary - (ele.salary * 0.01));
}
