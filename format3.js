...
function findWeakColumns(matrix, limit) {
  ...
  for (let i = 0; i < matrix[0].length; i++) {
    let colSum = matrix.reduce((occ, current) => occ + current[i], 0);
    if (colSum < limit) {
      result.push({ col: i, total: colSum });
    }
  }

  return result;
}
console.log(findWeakColumns(matrix, 2));
...
// console.log(isMirror(matrixx[0], matrixx[1]));
function findMirrorRows(matrix) {
  // votre code
...
console.log(findMirrorRows(matrixx));
