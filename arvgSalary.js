...
function avgSalary(obj) {
  ...
  for (let i = 0; i < obj.length; i++) {
    const department = obj[i].department;
    if (!result[department]) {
      result[department] = { salary: obj[i].salary };
    }
    result[department].salary += obj[i].salary;
  }
  return result;
...
function getHighestPaid(obj) {
    const activeEmployees = obj.filter((ele) => ele.isActive === true);
    
    for(let i = 0; i < obj.length; i++)
        {
            for(let o = i + 1; o < obj.length; o++){
                if(obj[i].salary < obj[o].salary)
                    {
                        const temp = obj[i].salary;
                obj[i].salary = obj[o].salary;
                obj[o].salary = temp;
            }
        }
    }
    
    const topFive = activeEmployees.slice(0, 5);
    return topFive;
}
